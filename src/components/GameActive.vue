          ...
          <div v-for="(p, i) in participants" :key="p.id"
            ...
            <div class="relative w-14 h-14 shrink-0">
              <div class="w-full h-full rounded-full flex items-center justify-center text-white text-xl font-bold shadow-sm transition-all"
                :class="[
                  avatarColors[i % 4],
                  (p.id !== currentParticipant?.id || (phase === 'idle' && !spinResult)) && getPlayerEffects(p.id).length > 0 ? 'ring-4 ' + effectMeta(getPlayerEffects(p.id)[0]?.effectCode ?? '').ring : ''
                ]">
                {{ p.displayName.charAt(0).toUpperCase() }}
              </div>
              <!-- Badge coin haut-droite -->
              <span v-if="(p.id !== currentParticipant?.id || (phase === 'idle' && !spinResult)) && getPlayerEffects(p.id).length > 0"
            ...
            <!-- Badge effet visible -->
            <div v-if="(p.id !== currentParticipant?.id || (phase === 'idle' && !spinResult)) && getPlayerEffects(p.id).length > 0"
